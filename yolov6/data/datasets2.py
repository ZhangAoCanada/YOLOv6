...
class TrainValDataset(Dataset):
    ...
    def generate_label_maps(self, ):
        self.label_maps = {}
        for i, name in enumerate(self.class_names):
            self.label_maps[name] = i
    ...
    def check_label_files(args):
        ...
        try:
            ...
            return img_path, lb_path, nc, nm, nf, ne, msg
        except Exception as e:
            nc = 1
    ...
    def generate_coco_format_labels(img_info, class_names, save_path):
        ...
        for i, class_name in enumerate(class_names):
            ...
            label_maps[class_name] = i

        ann_id = 0
        LOGGER.info(f"Convert to COCO format")
        for i, (img_path, info) in enumerate(tqdm(img_info.items())):
            labels_raw = annotation_from_xml(info["labels"]).to_yolo(label_maps)
            labels = np.array([list(map(float, x.split())) for x in labels_raw.split("\n") if x.strip()])
            labels = np.array(labels, dtype=np.float32)
            # print("====> [DEBUG] labels.shape: ", labels.shape)
            img_id = osp.splitext(osp.basename(img_path))[0]
            img_h, img_w = info["shape"]
            # print("====> [DEBUG] img_h: {}, img_w: {}".format(img_h, img_w))
            ...
            if len(labels):
                for label in labels:
                    ...
                    x = int((x1 + x2) / 2)
                    y = int((y1 + y2) / 2)
                    info_labels_new.append([cls_id, x, y, w, h])
            info["labels"] = info_labels_new
