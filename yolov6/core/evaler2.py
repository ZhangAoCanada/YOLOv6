...
from yolov6.data.data_load2 import create_dataloader
...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task, plot_imgs=True):
        '''Model prediction
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            # if plot_imgs:
            #     det = outputs.copy()
            #     for image, shap, label, pred in zip(imgs, shapes, targets, det):
            #         if len(pred) == 0:
            #             pred[:, :4] = self.rescale(image.shape[2:], det[:, :4], shap)
            ################################################################
